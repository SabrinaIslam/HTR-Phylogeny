...
- Chronogram input in .tre format generated from BEAST2 and summarised by TreeAnnotator 
...
- We will remove the Dro sequence and create a new df

- We will shorthand the species name for more readable plots

- We will add a column for evolutionary distance from the human (split time in MYA unit)
...
# Motif and Microswitch

- X axis protein 

- Y axis orthologue 

- geom_line does not give the desired plot, maneuvered geom_bar to be thinner
...
- Pre-analysis: prepping the input from the .tre file by manually renaming the taxa. Of course this will create duplicates as I will remove the receptor prefix from each taxa and it will just be species names. So it will no longer be read by Figtree. I will load every step of manually edited .tre in R  

- revts() is not working after updating so I shifted revts before the geom_hilight()
